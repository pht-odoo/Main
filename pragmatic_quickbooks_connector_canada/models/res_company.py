...
class ResCompany(models.Model):
                                    ...
                                    if res_partner:
                                        ...
                                        if cust.get('GlobalTaxCalculation'):
                                            if cust.get('GlobalTaxCalculation') == 'TaxExcluded':
                                                ...
                                                dict_s['tax_state'] = 'notapplicable'
    ...
    def import_purchase_order(self):
        ...
        if company.access_token:
            ...
            data = requests.request('GET', company.url + str(company.realm_id) + "/query?query=" + query +"&minorversion="+company.minorversion,
                                    headers=headers)
            if data.status_code == 200:
                recs = []
                parsed_data = json.loads(str(data.text))
                _logger.info(_('\n\n Parsed Dict : %s' % parsed_data))
                if parsed_data:
                                    ...
                                    for i in cust.get('Line'):
                                        if i.get('ItemBasedExpenseLineDetail'):
                                            ...
                                            if res_product:
                                                ...
                                                if i.get('ItemBasedExpenseLineDetail').get('TaxCodeRef'):
                                                    tax_id = self.env['account.tax'].search([('qbo_tax_id', '=', int(i.get('ItemBasedExpenseLineDetail').get('TaxCodeRef').get('value'))),
                                                                                             ('type_tax_use', '=', 'purchase')], limit=1)
                                                    # print('______tax_id_________', tax_id)
                                                    if tax_id:
                                                        dict_l['taxes_id'] = [[6, False, [tax_id.id]]]

                                                # print("Order Line : ", dict_l)
